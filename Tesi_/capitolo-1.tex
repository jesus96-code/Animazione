...
Nel contesto dell'animazione 3D, ogni elemento visivo - sia esso un personaggio o un oggetto - prende vita attraverso una combinazione di trasformazioni geometriche fondamentali: traslazione, rotazione s scalatura. Tali operazioni sono essenziali per finire le posizione, l'orientamento e la dimensione della mesh nello spazio tridimensionale, e vengono efficacemente rappresentate grazie all'uso di matrici 4x4 in coordinate omogenee. Queste matrici permettono la composizione sequenziale di più trasformazioni, garantendo un controllo elegante e coerente della deformazione e del movimento dei modelli.
...
Di seguito vengono descritte le tre principali trasformazioni elementari che definiscono il comportamentp spaziale di una mesh o di un rig:
...
Per la rappresentazione delle rotazioni, un approccio tradizionale è quello basato sugli angoli di Eulero, i quali definiscono l'orientamento di un corpo attraverso una sequenza ordinata di rotazioni elementari attorno agli assi principali del sistema di riferimento (X, Y, Z). Nonostante la loro intuitività, gli angoli di Eulero presentano alcune limitazioni pratiche, tra cui il problema di gimbal lock, che comporta la perdita di un grado di libertà nella rotazione quando due assi di rotazione si allineano.
...
\begin{itemize}
    ...
    Questa rappresentazione è intuitiva, poiché ogni angolo corrisponde a una rotazione attorno a un assi specifico, permettendo di comprendere facilmente il movimento nello spazio tridimensionale . Gli angoli di Eulero sono stati ampiamente utilizzati in diversi ambiti, tra cui la grafica computazionale, la robotica e l'ingegneria aerospaziale, grazie alla loro semplicità e compatibilità con numerosi software e framework.

    Tuttavia, questo metodo presenta anche alcune limitazioni strutturali. In particolare, soffre del fenomeno noto come gimbal lock, che si verifica quando due assi di rotazioni si allineano, provocando la perdita di un grado di libertà e rendendo instabile la rappresentazione. Inoltre, la dipendenza dall'ordine delle rotazioni (ad esempio, XYZ o ZYX) può portare a risultati diversi per la stessa configurazione, generando ambiguità nella definizione dell'orientamento.
    Per questi motivi, pur rimanendo un approccio intuitivo e facilmente interpretabile, gli angoli di Eulerosono oggi spesso sostituiti dai quaternioni, che offrono una rappresentazione più stabile e continua delle rotazioni nello spazio tridimensionale ~\cite{quater}.
    ...
    La rappresentazione tramite asse e angolo, tuttavia, \textbf{non è univoca}: la stessa rotazionale può essere ottenuta scalando il vettore asse e modificando l'angolo di un multiplo 2${\pi}$, come si evidenza in figura ~\ref{fig:asse2}. Inoltre, la coppia (${v}$, ${\theta}$) produce la stessa rotazione della coppia (-${v}$ , -${\theta}$).
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            | L_q(v) | = |v|
        \end{align*}
        \item \textbf{Lacia invariati i vettori paralleli all'asse di rotazione}: se v è proporzionale a u, allora ${L_q(v) = v}$
    \end{enumerate}

    Queste proprietà mostrano ${L_q}$ rappresenti esattamente una \textbf{rotazione nello spazio tridimensionale} ~\cite{quater3}.
    Più precisamente, per qualsiasi quaternioni unitario:
    \begin{align*}
        q = {\cos{(\theta/2)} + u\sin{(\theta/2)}}
    \end{align*}
    l'operazione q v ${p^*}$ applica al vettore ${v}$ un rotazione di ampiezza ${\theta}$ attorno all'asse ${u}$.

    Gli angoli di Eulero, pur offrendo una rappresentazione intuitiva delle rotazioni tridimensionali, presentano diversi limiti che ne riducano l'efficacia in contesti complessi come l'animazione digitale  la robotica. Il principale problema è il fenomeno noto come gimbal lock, una condizione in cui due assi di rotazione diventano allineati, riducendo di fatto i gradi di libertà e impedendo di rappresentare alcune orientazioni in modo univoco.

    Inoltre, le rotazioni espresse tramite angoli di Eulero sono dipendenti dall'ordine di applicazione (ad esempio XYZ, ZYX, ecc.) e questo comporta ambiguità nella rappresentazione dell'orientamento di un oggetto. Tale dipendenza può generare instabilità numeriche e comportamenti imprevisti, sopratutto quando le rotazioni vengono interpolate nel tempo - una condizione molto comune nei sistemi di animazione.
...
Quando si utilizzano dati di motion capture, può essere necessario adattare lo scheletro affinchè rispetti le proporzioni della mesh, per poi traferire (retargeting) il movimento originale al nuovo scheletro.
...
Per far muovere la mesh insieme allo scheletro si utilizza un processo detto skinning, dove ogni vertici è associato a uno o più giunti tramite pesi di influenza:
\begin{align*}
...
Negli ultimi anni, il campo del motion retargeting ha visto la diffusione di metodi automatici capaci di trasferire animazioni tra personaggi con proporzioni corporee o strutture scheletriche differenti, riducento al minimo l'interveto manuale. Questi approcci sfruttano modeli statistic o reti neurali per apprendere la relazione tra pose, scheletri e deformazioni della mesh, con l'obiettivo di preservare la naturalezza del movimento durante il trasferimento.
...
approcci come ASMR utilizzano reti neurali per apprendere direttamente il processo id retargeting. Il modello  è addestrato su numerosi esempi di movimenti eseguiti da scheletri differenti, imparando a mantenere la coerenza del gesto indipendentemente dalla struttura del personaggio.
Rispetto ai metodi puramente geometrici, tali tecniche possono addattare automaticamente lo stile del movimento e ridurre distorsioni o artefatti durante la deformazione.
